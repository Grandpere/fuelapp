...
final readonly class AnalyticsFuelPricePerMonthStateProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): object|array
    {
        ...
        $items = $this->kpiReader->readFuelPricePerMonth(
            $ownerId,
        ...
        foreach ($items as $item) {
            $id = $item->month.'-'.$item->fuelType;
            $result[] = new AnalyticsFuelPricePerMonthOutput(
                $id,
                $item->month,
                $item->fuelType,
                $item->totalCostCents,
                $item->totalQuantityMilliLiters,
                $item->averagePriceDeciCentsPerLiter,
            );
        }
