...
final readonly class AnalyticsVisitedStationsStateProvider implements ProviderInterface
{
    ...
    public function provide(Operation $operation, array $uriVariables = [], array $context = []): object|array
    {
        ...
        $items = $this->kpiReader->readVisitedStations(
            $ownerId,
        ...
        foreach ($items as $item) {
            $result[] = new AnalyticsVisitedStationOutput(
                $item->stationId,
                $item->stationName,
                $item->streetName,
                $item->postalCode,
                ...
                $item->latitudeMicroDegrees / 1_000_000,
                $item->longitudeMicroDegrees / 1_000_000,
                $item->receiptCount,
                $item->totalCostCents,
                $item->totalQuantityMilliLiters,
            );
        }
